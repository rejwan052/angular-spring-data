...
abstract class AbstractBaseTableFilter<T, F> implements IBaseTableFilter<T, F> {
	...
	protected AbstractBaseTableFilter(JunctionType junctionType) {
	...
	private boolean applyLikePostFilter(PostFilterItem<T> postFilterItem, T element) {
        String filterString = AbstractFilterSpecification.convertToRegexp(AbstractFilterSpecification.processFilterString(postFilterItem.getFilterValue().toString()));					
		Object value = getResolvedValue(postFilterItem, element);
		try {
