...
import skyglass.data.filter.AbstractTableFilter;
...
import skyglass.data.filter.HibernateFilterItem;
import skyglass.data.filter.JunctionType;
import skyglass.data.filter.OrderField;
import skyglass.data.query.QueryResult;

public abstract class AbstractHibernateTableFilter<T> extends AbstractTableFilter<T, TableFilter<T>> implements TableFilter<T> {
	...
	public FilterItem createFilterItem(String fieldName, FieldType fieldType, Object filterValue) {
		return new HibernateFilterItem(filterSpecification, clazz, getFieldResolver(fieldName, fieldType), filterValue);
	}     
    
	public FilterItem createFilterItem(String fieldName, FieldType fieldType, Object filterValue, FilterType filterType) {
		return new HibernateFilterItem(filterSpecification, clazz, getFieldResolver(fieldName, fieldType), filterValue, filterType);
	}     
    
	public FilterItem createFilterItem(String fieldName, FieldType fieldType, Object filterValue, FilterClass filterClass) {
		return new HibernateFilterItem(filterSpecification, clazz, getFieldResolver(fieldName, fieldType), filterValue, filterClass);
	}      
	...
	public FilterItem createFilterItem(String fieldName, FieldType fieldType, Object filterValue, FilterType filterType, FilterClass filterClass) {
		return new HibernateFilterItem(filterSpecification, clazz, getFieldResolver(fieldName, fieldType), filterValue, filterType, filterClass);
	}    
    
    public AbstractHibernateTableFilter(Class<?> clazz, JunctionType junctionType, 
    		AbstractFilterSpecification filterSpecification) {
        super(junctionType);
        this.clazz = clazz;
    ...
    public Criteria createRootCriteria() {
    	return filterSpecification.createRootCriteria(clazz);
    }
    ...
    protected void resolveCustomFilter(CustomFilterResolver filterResolver) {
   		((CustomHibernateFilterResolver)filterResolver).addCustomFilter(rootCriteria);
    }
    ...
    private String concat(String concat1, String concat2) {
    	StringBuilder sb = new StringBuilder();
    	if (filterSpecification.getDialect() instanceof DB2Dialect) {
    ...
    private static class CustomIlikeExpression extends IlikeExpression
    {
    	...
    	private AbstractFilterSpecification filterUtils;    	
    	
		public CustomIlikeExpression(Class<?> rootClass, String propertyName, Object value, AbstractFilterSpecification filterUtils)
		{
		...
		public String toSqlString(Criteria criteria, CriteriaQuery criteriaQuery) {
			Dialect dialect = filterUtils.getDialect();
			...
			if ((dialect instanceof DB2Dialect) 
					&& filterUtils.isNumericField(rootClass, propertyName)) {
    ...
    private String formatFieldToSql(String expression) {
    	...
    	return filterSpecification.getDialect().getLowercaseFunction() + '(' + "{" + currentAlias + "}." + camelCaseToUnderscore(propertyName) + ")";
    } 
    
    public String resolveAliases(String expression) {
    	expression = AbstractFilterSpecification.normalizeFieldName(expression);
    	String[] values1 = expression.split("\\.");
    	if (values1.length == 1) {
	...
	private Criterion applyLikeFilter(String fieldName, Object filterValue) {
        return new CustomIlikeExpression(clazz, fieldName, filterValue.toString(), filterSpecification); 
	}
